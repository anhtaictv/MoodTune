...
| --- | --- | --- |
| `v3.8` | Giao diện đa theme | Thêm bộ chọn giao diện ngay trong app với 3 theme: `Tối · Midnight` (mặc định), `Sáng · Aurora` (nền trắng, sương màu pastel, card nổi bằng shadow) và `Rực rỡ · Sunset` (gradient hoàng hôn chuyển động nhẹ, card kính mờ/glass) — áp dụng ngay, tự nhớ qua `localStorage`. Bỏ anti-pattern gradient-text ở logo/heading, chuẩn hoá lại màu chữ/nền để đạt độ tương phản WCAG AA ở cả 3 theme, và sơ đồ tri thức AI (canvas) giờ đọc màu theo theme đang chọn thay vì hardcode cố định xanh-tím. |
